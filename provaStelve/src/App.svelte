<script>
    ...
    function removerTarefa(tarefaRemover) {
        data = data.filter((t) => t.id !== tarefaRemover.id);
    }
...
<main>
    ...
    <Tabela {data} {open} remover={removerTarefa} check={tarefaConcluida} />

    {#if show}
        <div transition:fade>
            <Dialogo {close} tarefa={tarefaSelecionada} />
        </div>
    {/if}
</main>
